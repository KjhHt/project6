...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <section class="content">
            ...
            <div class="card-body">
             <form action="${path}/uptIssuespage.do" class="form" method="post">
              ...
              <div class="form-group">
                ...
                <input type="text" name="ititle" value="${is.ititle}" class="form-control">
              </div>
              <div class="form-group">
                <label for="inputDescription">이슈사항 내용</label>
                <textarea name="icontent" class="form-control" rows="4">${is.icontent}</textarea>
              </div>              
              <div class="form-group">              
				<label for="inputDescription">최근 수정일</label>
			<input  class="form-control" 
				value='<fmt:formatDate value="${is.iuptdate}" pattern="yyyy-MM-dd hh:mm:ss"/>' placeholder="작성자 입력하세요" />	
              </div>
              <div class="form-group"> 
              	<label for="inputProjectLeader">처리현황</label>
                <select name="iprogress" class="form-control pm-select">
                  <option value="PROG" <c:if test="${is.iprogress=='PROG'}">selected</c:if>>진행</option>
                  <option value="REJ" <c:if test="${is.iprogress=='REJ'}">selected</c:if>>반려</option>
                  <option value="REQ" <c:if test="${is.iprogress=='REQ'}">selected</c:if>>대기</option>
                  <option value="COMP" <c:if test="${is.iprogress=='COMP'}">selected</c:if>>처리완료</option>
                </select> 
              </div>
			  </form>
            </div>
      ...
      <div class="row">
        <div class="col-12">
          <a href="jobcheck.do?pid=${param.pid}" class="btn btn-secondary">취소</a>  
          <button type="button" onclick="treatProc()" class="btn btn-primary float-right">처리</button>
        </div>
      </div>
...
<script type="text/javascript">
	function treatProc(){
		$("form").attr("action","${path}/treatIssue.do");
		$("form").submit();
	}
